  ...
  void SoftmaxWithLossLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
      ...
      /*if (normalization_ == LossParameter_NormalizationMode_VALID &&
        ...
        if (count != Dtype(0.0))
          loss /= get_normalizer(normalization_, count);
      } else {
        loss /= outer_num_;
      }*/
      Dtype weights_sum;
      caffe_gpu_asum(nthreads, counts, &weights_sum);
      top[0]->mutable_cpu_data()[0] = weights_sum == Dtype(0.0) ? Dtype(0.0) : (loss / get_normalizer(normalization_, weights_sum));
  ...
  void SoftmaxWithLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
      ...
      if (propagate_down[0]) {
        ...
        Dtype weights_sum;
        caffe_gpu_asum(nthreads, counts, &weights_sum);
        caffe_gpu_scal(prob_.count(), weights_sum == Dtype(0.0) ? Dtype(0.0) : (loss_weight / get_normalizer(normalization_, weights_sum)), bottom_diff);
      }
  }
