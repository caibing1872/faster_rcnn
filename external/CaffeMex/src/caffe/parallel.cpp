...
namespace caffe {
    ...
    for (int d = 0; d < remaining_depth; ++d) {
      for (int i = 0; i < remaining.size(); ++i) {
        for (int j = i + 1; j < remaining.size(); ++j) {
          int access;
          CUDA_CHECK(
            cudaDeviceCanAccessPeer(&access, remaining[i], remaining[j])); // check if GPUx can access GPUy's mem directly
          if (access) {
            pairs->push_back(DevicePair(remaining[i], remaining[j]));
  ...
  P2PSync<Dtype>::P2PSync(shared_ptr<Solver<Dtype> > root_solver,
    ...
    solver_() {
      ...
      if (parent == NULL) {
        ...
        // construct WorkerSolver, the constructor will call Solver's constructor,
        //to prevent it construct test_net_, set root_solver_ to false
        solver_.reset(new WorkerSolver<Dtype>(param, root_solver.get()));
        Caffe::set_root_solver(true);
      }
      this->configure(solver_.get());
      solver_->add_callback(this); // signal to every process (solver.hpp 32)
  ...
  void P2PSync<Dtype>::on_start() {
	...
	printf("card %d : on_start cudeGetDev(%d) vs solver_(%d)\n",device,device,solver_->param().device_id());
  ...
  void P2PSync<Dtype>::on_gradients_ready() {
    ...
    int device;
    CUDA_CHECK(cudaGetDevice(&device));
	printf("card %d : on_gradients cudeGetDev(%d) vs solver_(%d)\n",device,device,solver_->param().device_id());
    CHECK(device == solver_->param().device_id());
#endif
    ...
    for (int i = 0; i < children_.size(); ++i) {
	  ...
	  {
	  	printf("thread_resource error! %s\n",e.what());
		return;
	  }
	  catch(boost::thread_interrupted e)
	  {
#ifdef DEBUG
		  printf("card %d interrupted exception!!\n",device);
#else
		  printf("interrupted exception!!\n");
#endif
		  return;
	  }
	  catch(...)
...
#ifdef DEBUG
	  printf("card %d push himself into parent's queue!\n",device);
#endif
    }
    else {
#ifdef DEBUG
	  printf("card %d gpu_scal\n",device);
#endif
  ...
  template<typename Dtype>
  void P2PSync<Dtype>::pre_run(const vector<int>& gpus)
  {
    // Pair devices for map-reduce synchronization
    vector<DevicePair> pairs;
    DevicePair::compute(gpus, &pairs);
    ostringstream s;
    s << "pair_size(" << pairs.size() << ") ";
      ...
      for (int i = 1; i < pairs.size(); ++i) {
        if (!workers_[i].get()) {
          ...
          for (int j = 0; j < workers_.size(); ++j) {
            P2PSync<Dtype>* sync = j == 0 ? this : workers_[j].get();
            if (sync) {
          ...
          if (parent) {
            ...
            workers_[i].reset(new P2PSync<Dtype>(solver_, parent, param));
            parent->children_.push_back((P2PSync<Dtype>*) workers_[i].get());
          }
        }
      }
    }
  ...
  void P2PSync<Dtype>::run(const vector<int>& gpus) {
    ...
    for (int i = 1; i < workers_.size(); ++i) {
      workers_[i]->StartInternalThread(); //create a thread call @override-InternalThreadEntry(274) (internal_thread.cpp 21)
    }
