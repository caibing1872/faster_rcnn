...
%
% hyli: binary version, only have two classes (bg/fg)
% --------------------------------------------------------
...
labels_blob(labels_blob > 0) = 1; % hyli: change FCN to binary lable (fg and bg)
...
% change the following in this binary version
bbox_target_data = full(image_roidb.bbox_targets(keep_inds, :));
clss = bbox_target_data(:, 1);
bbox_targets = zeros(length(clss), 4 * (1+1), 'single');
bbox_loss_weights = zeros(size(bbox_targets), 'single');
inds = find(clss > 0);
for i = 1:length(inds)
    ind = inds(i);
    bbox_targets(ind, 5:8) = bbox_target_data(ind, 2:end);
    bbox_loss_weights(ind, 5:8) = 1;
end
end
